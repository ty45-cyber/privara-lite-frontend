...
import { runSageAgent }         from '../lib/agents'
...
export default function Governance() {
  ...
  useEffect(() => {
    const propId = searchParams.get('proposal')
    const action = searchParams.get('action')
    if (!propId || proposals.length === 0) return
    if (action === 'tally') viewResults(propId)
  ...
  return (
    <div className="governance animate-fade-up">
      <PageHeader
        ...
        subtitle="Encrypted ballots · Anonymous tally · Sage AI proposal drafter"
        actions={
          canCreate && (
        ...
        {proposals.map(p => (
          <div key={p.id} className="proposal-card">
            ...
            <div className="proposal-actions">
              {p.status === 'active' && (
                <>
                  <Button variant="success" size="sm" onClick={() => castVote(p.id, 'yes')}     loading={voting[p.id]}>YES</Button>
                  <Button variant="ghost"   size="sm" onClick={() => castVote(p.id, 'abstain')} loading={voting[p.id]}>ABSTAIN</Button>
                  <Button variant="danger"  size="sm" onClick={() => castVote(p.id, 'no')}      loading={voting[p.id]}>NO</Button>
                </>
              )}
              {canTally && (
      ...
      <Modal open={createModal} onClose={() => setCreateModal(false)} title="CREATE GOVERNANCE PROPOSAL">
        <div className="gov-form">
          {/* Sage AI — draft from title */}
          {form.title && (
            <AgentPanel
              agentName="SAGE"
              agentRole="Governance Proposal Drafter"
              trigger="DRAFT FULL PROPOSAL FROM TITLE"
              accentColor="green"
              onRun={(onToken, onDone) => runSageAgent(form.title, onToken, onDone)}
            />
          )}
          <Input
            label="TITLE"
            value={form.title}
            onChange={set('title')}
            placeholder="Approve treasury expansion…"
          />
          <Textarea
            label="DESCRIPTION"
            value={form.description}
            onChange={set('description')}
            placeholder="Detailed context for voters…"
          />
          <Input
            label="VOTING ENDS AT"
            type="datetime-local"
            value={form.voting_ends_at}
            onChange={set('voting_ends_at')}
          />
  ...
  return (
    <div className="tally-bar">
      ...
      <div className="tally-bar-track">
        <div className={`tally-bar-fill tally-bar-fill--${color}`} style={{ width: `${pct}%` }} />
      </div>
    </div>
  )
}
